...
### Scary red herring mid-session: board went dark (no power) during testing
- Partway through timing tests, the board stopped responding to *everything* — USB, WiFi, WebSerial, all silent — and the screen went black. Initially looked like the `serialCommandTask` fix itself had bricked something. Turned out to be a physical connection issue (unclear exactly what — cable/port, not investigated further since a reset immediately fixed it), unrelated to any code from this session. Worth remembering: if serial testing suddenly goes 100% silent (not just CHAN, but `READ` too, and WebSerial also unreachable), check the board is actually powered/connected before assuming it's a firmware regression.
